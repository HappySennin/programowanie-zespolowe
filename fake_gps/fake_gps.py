...
def main():
    ...
    while True:
        for car_id in cars_and_locations.keys():
            if len(cars_and_locations[car_id]) > 0:
                send_update_request(bearer_token, car_id, cars_and_locations[car_id][0])
                cars_and_locations[car_id] = cars_and_locations[car_id][1:]
            else:
                log("no more locations for car id {}".format(car_id))
